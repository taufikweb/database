...
# Konfigurasi**
# ---------------------------------------------#
# Waktu periksa ulang dalam hitungan detik untuk memeriksa kembali semua akun (60 detik = 1 menit dan 0 berarti tidak ada pemeriksaan ulang)
AccountsRecheckTime = 30

# Menambahkan penundaan acak ke interval AccountsRecheckTime untuk membuatnya lebih tidak terduga dan kurang terdeteksi.
# Setel ke 0 untuk menonaktifkan penundaan acak.
# Misal jika diset ke 120 maka bot akan intro
MaxRandomDelay = 30

# Akun akan diperiksa sesuai urutannya
AccountList = [
    {
        "account_name": "RinneJay: ",  # A custom name for the account (not important, just for logs)
        "Authorization": "Bearer 1719608998748ljGQVQB9EgWYWatDbsfWZh2cNSveFxRsIZpBmYKnmE7EUk9eKGzoZFwgX3M0LApM1648462953",  # To get the token, refer to the README.md file
        "UserAgent": "Mozilla/5.0 (Linux; Android 11; RMX2193) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36",  # Refer to the README.md file to obtain a user agent
        ...
        "config": {
            ...
            "auto_upgrade_start": 2000000000,  # Start buying upgrades when the balance is greater than this amount
        ...
        "telegram_chat_id": "1648462953",  # String - you can get it from https://t.me/chatIDrobot
        #"telegram_chat_id": "6267516678",  # String - you can get it from https://t.me/chatIDrobot
        "telegram_group_id": "-1002022377187,",
    },
...
telegramBotLogging = {
    ...
    "bot_token": "7186399754:AAGIIVUsXIbHzQHBwKdEQHNP2rPC2TWS7wY",  # HTTP API access token from https://t.me/BotFather ~ Start your bot after creating it
    # Configure the what you want to receive logs from the bot
    "messages": {
        "general_info": True,  # Informasi Umum
        "account_info": True,  # Informasi Akun
        "http_errors": False,  # kesalahan HTTP
        "other_errors": False,  # Kesalahan lainnya
        "daily_cipher": True,  # Sandi harian
        "daily_task": False,  # Tugas sehari hari
        "upgrades": True,  # Peningkatan
    },
}
...
#LOGFORMAT = "%(log_color)s[HamsterKombatBot]%(reset)s[%(log_color)s%(levelname)s%(reset)s] %(log_color)s%(message)s%(reset)s"
...
class HamsterKombatAccount:
    def __init__(self, AccountData):
        ...
        self.telegram_group_id = AccountData["telegram_group_id"]
        #self.telegram_chat_id = 
    def telegramCipherLog(self, message, level):
        #print(message)
        if (
        ...
        requests.get(
            f"https://api.telegram.org/bot{telegramBotLogging['bot_token']}/sendMessage?chat_id={self.telegram_group_id}&text={message}&reply_to_message_id=109&type=supergroup&message_thread_id=109&parse_mode=MarkdownV2&disable_web_page_preview=true"
        )
    def SendTelegramLog(self, message, level):
        ...
        requests.get(
            f"https://api.telegram.org/bot{telegramBotLogging['bot_token']}/sendMessage?chat_id={self.telegram_group_id}&text={message}&reply_to_message_id=109&type=supergroup&message_thread_id=109"
        )
    # Send HTTP requests
    def HttpRequest(
        self,
    ...
    def Start(self):
        ...
        if (
            ...
            self.telegramCipherLog(
                f"[{self.account_name}] Decoding daily cipher...",
                "account_info"
            )
            DailyCipher = DailyCipherDecode(AccountConfigData["dailyCipher"]["cipher"])
            MorseCode = TextToMorseCode(DailyCipher)
            log.info(
                f"\033[1;34m[{self.account_name}] Daily cipher: {DailyCipher} \nMorse code: \n{MorseCode}\033[0m"
            )
            self.telegramCipherLog(
                f"[{self.account_name}] Sandi Harian sekarang: `{DailyCipher}` ```\n{MorseCode}\n``` Kunjungi: [HamsterKombatBot](https://t.me/haMster_kombat_bot/start?startapp=kentId1648462953)",
                "account_info",
            )
        ...
        if self.config["auto_tap"]:
            ...
            self.SendTelegramLog(
                f"[{self.account_name}] Koin Kamu: {number_to_string(self.balanceCoins)} coins,\n[{self.account_name}] Keuntungan Per Jam: {number_to_string(self.earnPassivePerHour)}","account_info"
            )
